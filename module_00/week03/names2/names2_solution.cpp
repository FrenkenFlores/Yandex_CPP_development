...
GetFullNameWithHistory. Но для того, чтобы избежать дублирования кода
в методах GetFullName и GetFullNameWithHistory, решение предыдущей задачи
...
Функцию BuildJoinedName, по истории изменений имени (фамилии) возвращающую
эту историю в виде единой строки: сначала последнее имя, затем в круглых скобках через запятую все предыдущие имена.

Функцию BuildFullName, по имени и фамилии (возможно, неизвестным, то есть пустым)
возвращающую полное имя в требуемом формате. В прошлой задаче этот код находился непосредственно в методе GetFullName, теперь же он понадобился и в методе GetFullNameWithHistory — потому мы и выделили его в отдельную функцию.

Кроме того, напишем короткие вспомогательные приватные методы FindFirstNamesHistory
и FindLastNamesHistory, с помощью функции FindNamesHistory возвращающие по данному году историю изменений имени или фамилии.

В методах GetFullName и GetFullNameWithHistory останется лишь аккуратно вызвать
описанные выше функции и методы.
*/
