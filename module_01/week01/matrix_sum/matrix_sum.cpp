...
public:
	...
	Matrix(const int num_rows, const int num_cols): _num_rows(num_rows), _num_cols(num_cols) {
		if (_num_rows < 0 || _num_cols < 0) throw std::out_of_range("Negative number of columns or rows");
		try {
	...
	void Rest(const int new_num_rows, const int new_num_cols) {
		// Clean old matrix
		// TODO: segfault
		if (_matrix) {
			for (int i = 0; i < _num_cols; i++) {
		...
		if (new_num_rows == 0 && new_num_cols == 0) {
			...
			*this = Matrix(new_num_rows, new_num_cols);
		}
	}

	int At(const int num_row, const int num_col) const {
		if (num_row >= _num_rows || num_col >= _num_cols) throw std::out_of_range("Out of range");
...
Matrix operator+(Matrix &matrix1, Matrix &matrix2) {
	if (matrix1.GetNumColumns() != matrix2.GetNumColumns() || matrix1.GetNumRows() != matrix2.GetNumRows()) {
		throw std::invalid_argument("Matrix have differ in size");
	}
